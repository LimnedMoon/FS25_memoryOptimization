...
function memoryOptimization:update(dt)
    ...
    if timer >= FULL_INTERVAL then
        ...
        if dbg then Logging.warning(string.format("[memoryOptimization] Gründliche 12-Min-Bereinigung: %.2f KB freigegeben | Aktueller RAM verfügbar: %.2f KB", cleaned, ramAfter)) end
    ...
    elseif timer >= nextMedium then
        ...
        if dbg then Logging.warning(string.format("[memoryOptimization] 3-Min-Zwischenbereinigung: %.2f KB freigegeben | Aktueller RAMverfügbar: %.2f KB", cleaned, ramAfter)) end
        nextMedium = nextMedium + MEDIUM_INTERVAL -- Schwellenwert um 3 Min erhöhen
    end
end

function memoryOptimization:loadMap(name)
    Logging.warning("[memoryOptimization] Mod aktiv - Intervalle: 3 Min / 12 Min")
    timer = 0
